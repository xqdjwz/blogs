...
export async function onRequest(context) {
  ...
  if (request.method === "GET") {
    try {
      const rows = await env.DB.prepare("SELECT key, value FROM config WHERE key IN ('site_title', 'site_subtitle', 'site_categories')").all();
      const configMap = {};
  ...
  if (request.method === "POST") {
    ...
    try {
      const { site_title, site_subtitle, site_categories } = await request.json();
